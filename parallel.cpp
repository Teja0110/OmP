...
double multiplyMatrix(int n) {
	...
	*/
	...
	cout << "Threads 1: " << omp_get_num_threads() << endl;
 	...
 	#pragma omp parallel for reduction(+:sum)  					//OpenMP parallel for
	for(int i = 0; i < n; i++){
	        for(int j = 0; j < n; j++){
	            sum = 0.0;
	            for (int k = 0; k < n; k++) {
...
double tiledMultiplication(int n) {
	...
	int x,y,z,i,j,k;
 	#pragma omp parallel for shared(matrixA,  matrixB, matrixC)  private(x, y, z, i, j, k, sum) schedule(static)          //OpenMP parallel for
	for (x = 0; x < n; x += tile) {
		for (y = 0; y < n; y += tile) {
			for (z = 0; z < n; z += tile) {
				for (i = x; i < x + tile; i++) {
